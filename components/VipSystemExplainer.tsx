...
import { VIP_TIERS, AFFILIATE_CONFIG } from "@/config/constants";
    ...
    <section id="how-carypact-works" className="py-14 sm:py-20 relative overflow-hidden bg-[#0A0C14] border-t border-[#1E243B]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="bg-[#0E1020] border border-amber-500/30 rounded-2xl p-5 mb-8 flex flex-col sm:flex-row items-start sm:items-center gap-4">
          ...
          <div className="space-y-1 text-xs text-[#C4CBD8]">
            ...
            <p className="leading-relaxed">
              The VIP reward mechanism functions as a multi-tier referral compensation model requiring personal capital (<strong className="text-white">300 USDT minimum</strong>) and aggregate downline &quot;leg&quot; volume (<strong className="text-white">≥3,000 USDT</strong>). Rewards depend directly on active network recruitment and team volume. These earnings are not passive fixed returns.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-white">{item.rewardPercent}</span>
                  </div>
                  {item.daoShare && (
                    <span className="text-[9px] uppercase font-extrabold bg-amber-500/20 text-amber-300 px-1.5 py-0.5 rounded">
                      DAO
                    </span>
                  )}
                </button>
          ...
          <div className="lg:col-span-8 bg-[#0E1020] border border-[#1E243B] rounded-3xl p-6 sm:p-8 shadow-2xl relative overflow-hidden">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-6 border-b border-[#1E243B]">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#22D3FF]">Active Performance Tier ({selectedTier.rewardPercent} Reward Share)</p>
                </div>
              </div>
              {selectedTier.daoShare && (
                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/20 text-xs font-bold text-amber-300">
                  ...
                  <span>Shares 3% DAO Pool (1,200 CA/day)</span>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 my-6">
              <div className="bg-[#131728] border border-[#1E243B] p-4 rounded-2xl">
                <div className="text-[11px] uppercase font-bold text-[#838E9E]">Personal Hashrate Requirement</div>
                <div className="text-xl font-extrabold text-white mt-1">{selectedTier.personalUSDT} USDT</div>
                <div className="text-[11px] text-[#838E9E] mt-0.5">300 Hashrate Units activated</div>
              </div>

              <div className="bg-[#131728] border border-[#1E243B] p-4 rounded-2xl">
                <div className="text-[11px] uppercase font-bold text-[#838E9E]">Team Leg Performance Volume</div>
                <div className="text-xl font-extrabold text-[#22D3FF] mt-1">
                  ${selectedTier.legUSDT.toLocaleString()} USDT
                </div>
                <div className="text-[11px] text-[#838E9E] mt-0.5">Cumulative downline team hashrate</div>
              </div>
            </div>

            {/* Tier Benefits */}
            <div className="bg-[#131728] border border-[#1E243B] p-4 rounded-2xl space-y-2">
              <div className="text-xs font-bold uppercase tracking-wider text-[#838E9E]">Included Protocol Perks</div>
              <div className="flex items-center gap-2 text-sm text-white">
                <Award className="w-4 h-4 text-emerald-400 flex-shrink-0" />
                <span>{selectedTier.perk}</span>
              </div>
